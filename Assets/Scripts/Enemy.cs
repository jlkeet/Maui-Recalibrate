...
public class Enemy : MonoBehaviour {
	...
	public float stunnedTime = 0.02f;   // Time stunned
	...
	public bool rattyGrounded = false;
	...
	void Update () {
		...
		rattyGrounded = Physics2D.Linecast(_transform.position, groundCheck.position, whatIsGround);
		_animator.SetBool ("EnemyIdle", rattyGrounded);
	...
	void FixedUpdate() {
		...
		Vector3 easeVelocity = _rigidbody.velocity;
		easeVelocity.y = _rigidbody.velocity.y;
		easeVelocity.z = 0.0f;
		easeVelocity.x *=0.75f;

		if (rattyGrounded) {
			_rigidbody.velocity = easeVelocity;
		}
...
//		// get the current scale
		...
		if ((_vx < 0f) && (localScale.x <= 0.5f)) {
			localScale = new Vector3 (-0.5f, 0.5f, 1.0f);
		} else if ((_vx > 0f) && (localScale.x >= -0.5f)) {
			localScale = new Vector3 (0.5f, 0.5f, 1.0f);
		}
//		// update the scale
...
//	}
	...
	public void Stunned()
	{
		Enemy ratty = gameObject.GetComponent<Enemy>();
		if (!isStunned) 
		{
			isStunned = true;
			...
			if (curHealth > 0) {
				StartCoroutine (ratty.Knockback (0.03f, 90.0f, ratty.transform.position));
			}
//			// stop moving
//			_rigidbody.velocity = new Vector2(0, 0);
